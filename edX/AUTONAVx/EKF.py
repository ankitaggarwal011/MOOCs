...
class UserCode:
    ...
    def calculatePredictMeasurementJacobian(self, x, marker_position_world, marker_yaw_world):
        ...
        #H = np.array([[-cos(x[2][0]),-sin(x[2][0]),-sin(x[2][0])*(marker_position_world[0][0] - x[0][0]) + cos(x[2][0])*(marker_position_world[1][0] - x[1][0])],[sin(x[2][0]),-cos(x[2][0]),-cos(x[2][0])*(marker_position_world[0][0] - x[0][0]) - sin(x[2][0])*(marker_position_world[1][0] - x[1][0])],[0,0,-1]])
        s_yaw = math.sin(x[2])
        c_yaw = math.cos(x[2])
